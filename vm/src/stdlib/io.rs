...
mod _io {
    ...
    use crate::builtins::{
        int, iter, pybool, pytype, PyByteArray, PyBytes, PyBytesRef, PyStr, PyStrRef, PyTypeRef,
    };
    ...
    fn new_unsupported_operation(vm: &VirtualMachine, msg: String) -> PyBaseExceptionRef {
        vm.new_exception_msg(vm.class("_io", "UnsupportedOperation"), msg)
    }
    ...
    trait BufferedMixin: PyValue {
        ...
        fn repr(zelf: PyObjectRef, vm: &VirtualMachine) -> PyResult<String> {
            let name = match vm.get_attribute(zelf.clone(), "name") {
                Ok(name) => Some(name),
                Err(e) if pytype::isinstance(&e, &vm.ctx.exceptions.value_error) => None,
                Err(e) => return Err(e),
            };
    ...
    pub(crate) fn extend_more(vm: &VirtualMachine, module: &PyObjectRef) {
        ...
        let unsupported_operation = pytype::new(
